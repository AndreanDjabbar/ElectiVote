...
`, resetURL)
	...
	go func() {
		...
		if err != nil {
			logger.Warn("Error sending email: %v", err)
		}
	}()
	logger.Info("ForgotPasswordPage - Email sent")
...
func ViewResetPasswordPage(c *gin.Context) {
	...
	if err != nil {
		logger.Warn("ViewResetPasswordPage - Invalid Token")
		utils.RenderError(
			c,
...
func ResetPasswordPage(c *gin.Context) {
	...
	email, err := verifyResetToken(token)
	if err != nil {
		logger.Warn("ResetPasswordPage - Invalid Token")
		utils.RenderError(
			c,
			http.StatusBadRequest,
			"Invalid Token",
	...
	if err != nil {
		logger.Warn("ResetPasswordPage - Internal Server Error")
		utils.RenderError(
			c,
